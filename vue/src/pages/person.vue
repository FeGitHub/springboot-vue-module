<template>
  <div>
    <Modal
      ...
      ref="modal"
    >
      <div slot="header" style="fontSize;fontWeight:bold">
      ...
      >
        ...
        <FormItem label="身份证号码" prop="identityNumber">
          <i-input
            ...
            style="width: 220px"
            :maxlength="30"
            v-model="ModalForm.identityNumber"
          ></i-input>
        </FormItem>
        ...
        <FormItem label="出生日期" prop="birthTime">
          <Date-picker
            placeholder="选择日期"
            style="width: 220px"
        ...
        <FormItem label="婚姻情况" prop="maritalStatus">
          <i-select
            placeholder="婚姻情况"
            style="width:220px;margin-right:10px;"
            :maxlength="2"
          ...
          >
            <i-option
              v-for="item in dicts.MARITAL_STATUS"
              :value="item.CODE"
              v-bind:key="item.CODE"
              >{{ item.NAME }}</i-option
            >
        ...
        <FormItem label="配偶姓名" prop="spouseName">
          <i-input
            ...
            style="width: 220px"
            :maxlength="15"
            v-model="ModalForm.spouseName"
          ></i-input>
        </FormItem>
      </Form>
      <div slot="footer">
        <div>
          <Button size="large" style="margin-left:8px" @click="close()"
            >关闭</Button
          >
          <Button
            type="primary"
    ...
    <div style="margin-bottom:20px;">
      ...
      />
      <RadioGroup v-model="queryForm.gender" @on-change="changeGender">
      ...
      <Input
        ...
        :maxlength="4"
      />
      <Button type="primary" @click="query('check')" style="margin-right:10px;"
        >查询</Button
      >
      <Button type="primary" @click="addPerson()">新建</Button>
    </div>
    <Table border ref="selection" :columns="columns" :data="pageData">
      <template slot-scope="{ row }" slot="PERSON_NAME">
      ...
      <template slot-scope="{ row }" slot="action">
        ...
        <Button
          ...
          >删除</Button
        >
        <Button type="primary" size="small" @click="viewPerson(row.ID)"
          >查看</Button
        >
      </template>
    </Table>
...
export default {
  ...
  methods: {
    ...
    checkBeforeSave () {
      ...
      if (
        dayjs(this.ModalForm.birthTime).format('YYYY-MM-DD') !=
        utils.getBirthday(this.ModalForm.identityNumber)
      ) {
